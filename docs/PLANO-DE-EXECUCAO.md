...
3. **Vercel.** Situação em 14/09/2026: variáveis cadastradas nos três ambientes; preset do Next.js e região gru1 no `vercel.json`; preview publicado pela CLI em https://schedule-optimizer-qnubrlx0b-leverproint.vercel.app (protegido pelo login da Vercel, só o time vê). Faltam três coisas, todas do lado do usuário:
   - dar ao app da Vercel acesso ao repositório `AlyGuimaraes/schedule-optimizer` (o `vercel git connect` continua falhando) **ou** criar um token na Vercel e gravá-lo como segredo `VERCEL_TOKEN` do repositório, que liga o job de deploy do CI;
   - antes de promover para produção, ligar **Deployment Protection › Vercel Authentication para todos os deployments** no projeto: sem o login da E03, a URL pública deixaria as ações de gravação abertas a qualquer pessoa;
   - confirmar o plano Pro do time, que a região gru1 exige.
   Promover: `vercel promote <url-do-preview>` ou `vercel deploy --prod`.
...
6. **CI com E2E.** O workflow já tem o passo: basta cadastrar `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` e `SUPABASE_SERVICE_ROLE_KEY` como segredos do repositório (`gh secret set NOME < arquivo`), de preferência de um projeto Supabase só de teste, porque dois testes gravam e limpam dados.
